...
if (!empty($search)) {
    $companies = $companyController->searchCompanies($_SESSION['user_id'], $search, $filter, $sort, $order);
} else {
    switch ($filter) {
        case 'my':
            $companies = $companyController->getMyCompanies($_SESSION['user_id'], $sort, $order);
            break;
        case 'recent':
