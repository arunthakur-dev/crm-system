...
try {
    ...
    if ($_POST['action'] === 'link_existing') {
        ...
        $dealController->linkDealToCompany($existing_deal_id, $company_id);
        ...
        $dealController->linkDealToCompany($new_deal_id, $company_id);
    }

    header("Location: /views/companies/view-company.php?contact_id=$company_id&success=1");
    exit;

} catch (Exception $e) {
    header("Location: /views/companies/view-company.php?contact_id=$company_id&error=" . urlencode($e->getMessage()));
    exit;
}
