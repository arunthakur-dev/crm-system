...
<div id="contactSidebar" class="sidebar sidebar-form hidden">
    ...
    <div class="tab-buttons">
        <button type="button" class="tab-btn active" data-tab="new">Create new</button>
        <button type="button" class="tab-btn" data-tab="existing">Add existing</button>
    </div>
    ...
    <div class="tab-content-wrapper">
        ...
        <form action="/includes/contacts/create-contact-inc.php" method="POST" class="tab-content active" id="new">
        ...
        <form action="/includes/contacts/link-contact-to-company.php" method="POST" class="tab-content" id="existing">
            <select name="existing_contact_id" required>
                ...
                <?php foreach ($allContacts as $contact): ?>
                    <option value="<?= $contact['contact_id'] ?>"><?= htmlspecialchars($contact['first_name']) ?> <?= htmlspecialchars($contact['last_name']) ?></option>
                <?php endforeach; ?>
            </select>
            <input type="hidden" name="company_id" value="<?= $company['company_id'] ?>">
            <div class="form-actions">
